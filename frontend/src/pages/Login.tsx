...
export function Login() {
  ...
  const handleSubmit = (e: SubmitEvent) => {
    ...
    const emailError = validateLoginId(loginId);
    const passwordError = validatePassword(password);

    if (emailError || passwordError) {
      setErrors({loginId: emailError, password: passwordError});
      return;
    }
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-16">
        <div className="w-full max-w-110">
          ...
          <div className="text-center mb-10">
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 mb-3">おかえりなさい</h1>
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-xl shadow-slate-100/50">
            <form onSubmit={() => handleSubmit} className="space-y-6" noValidate>
              ...
              <InputField
                ...
                onBlur={() => setErrors(prev => ({...prev, loginId: validateLoginId(loginId)}))}
                icon={<LuUser size={15}/>}
              />
